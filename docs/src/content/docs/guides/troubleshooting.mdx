...
**Fix**: this template currently keeps Hub metadata under `.runpod-disabled/` rather than `.runpod/`, so Hub auto-detection is disabled. When the folder is restored, keep `tests_.json` (underscore suffix) rather than `tests.json` if you want to skip validator execution.
